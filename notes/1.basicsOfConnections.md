...
## Application
 For this section, it would be a waste of time to look for the bad zig documentation, so better just ask ChatGPT about it, the better undestanding will be when writting the code
...
* `allocator` → para almacenar resultados
* `name` → IP o dominio
* `port` → puerto numérico

**Return**

* `AddressList`

  * contiene `.addrs` (slice de `std.net.Address`)
  * debes llamar `deinit()` cuando termines

---

## `std.posix.getaddrinfo`
...
* puntero a lista enlazada `addrinfo`
* liberar con `std.posix.freeaddrinfo()`
...
* `fd_t` → file descriptor del socket
...
* `sockfd` → socket en modo listen
* `addr` → donde se guarda info del cliente (puede ser null)
* `addrlen` → tamaño del sockaddr

**Return**

* `fd_t` → NUEVO socket conectado
...
* nada
* no devuelve error (si falla, es UB-level OS failure)
